...
class DownloadResult:
    ...
    def __init__(self, url: str, success: bool, file_path: Optional[str] = None, 
                 error: Optional[str] = None, size: int = 0, duration: float = 0.0):
        self.url = url
...
class DownloadProgress:
    ...
    def get_overall_progress(self) -> Tuple[int, int, float, float]:
        ...
        with self.lock:
            ...
            current_total = sum(prog[1] for prog in self.file_progresses.values() if prog[1] > 0)
...
class NetworkUtils:
    ...
    def __init__(self, timeout: int = 30, user_agent: str = None, max_concurrent: int = 5,
                 cache_dir: str = "temp/cache", cache_ttl_hours: int = 24):
        """
        初始化网络工具
    ...
    def _create_request(self, url: str, range_header: str = None) -> urllib.request.Request:
        """
        创建 HTTP 请求对象
    ...
    def _download_with_progress(self, url: str, output_path: Union[str, Path],
                               ...
                               file_id: str = None, use_cache: bool = True, 
                               support_resume: bool = True) -> Tuple[bool, int, float]:
        """
        下载文件并显示进度，支持缓存和断点续传
    ...
    def download_file(self, url: str, output_path: Union[str, Path], 
        ...
        last_error = None
        retry_delays = [retry_delay * (2 ** i) for i in range(max_retries + 1)]  # 指数退避
    ...
    def download_multiple(self, download_tasks: List[Tuple[str, Union[str, Path]]], 
                         ...
                         show_speed: bool = True) -> List[DownloadResult]:
        """
        ...
        def file_progress_callback(file_id: str, downloaded: int, total: int):
            ...
            # 获取整体进度信息
            completed, total_files, speed, elapsed = progress_tracker.get_overall_progress()
            
            if progress_callback:
                progress_callback(completed, total_files, file_id, speed, elapsed)
        
        def download_single(task_info: Tuple[int, Tuple[str, Union[str, Path]]]) -> DownloadResult:
            """下载单个文件的内部函数"""
    ...
    def download_multiple_with_stats(self, download_tasks: List[Tuple[str, Union[str, Path]]], 
                                   max_workers: int = None) -> Tuple[List[DownloadResult], Dict[str, Any]]:
        """
        并发下载多个文件并返回详细统计信息
