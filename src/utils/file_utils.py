...
class FileUtils:
    ...
    @staticmethod
    def write_text_file(
        path: Union[str, Path], lines: List[str], encoding: str = "utf-8"
    ) -> None:
        """
        写入文本文件
    ...
    def read_json_file(
        ...
        try:
            ...
            raise json.JSONDecodeError(f"JSON 格式错误 ({path}): {e.msg}", e.doc, e.pos)
    ...
    def cleanup_temp_files(
        temp_dir: Union[str, Path], keep_patterns: Optional[List[str]] = None
    ) -> int:
        """
        清理临时文件
