...
class ConverterService:
            ...
            try:
                ...
                for pattern, addresses in df_filtered.groupby('pattern')['address'].apply(list).to_dict().items():
                    ...
                    for addr in addresses:
                        if pd.notna(addr) and addr is not None:
                            addr_str = str(addr).strip()
                            if addr_str:  # 只添加非空字符串
                                stripped.add(addr_str)
                    
                    if not stripped:  # 如果没有有效地址，跳过
                        continue
    ...
    def convert_downloaded_ruleset(self, convert_name: str, download_data: 'DownloadedData') -> ConvertedData:
        """
        转换已下载的convert规则集数据
        ...
        """
        self.logger.info(f"🔄 开始转换规则集: {convert_name}")
        self.logger.info(f"📋 文件数量: {len(download_data.text_files)}")
        
        # 创建转换结果对象
        converted_data = ConvertedData(convert_name)
        converted_data.set_total_count(len(download_data.text_files))
        
        if not download_data.is_successful():
            self.logger.error(f"❌ 规则集 {convert_name} 下载失败，无法转换")
            converted_data.add_error("下载失败，无法转换")
            return converted_data
        ...
        for i, file_path in enumerate(download_data.text_files, 1):
            self.logger.info(f"🔄 转换文件 ({i}/{len(download_data.text_files)}): {Path(file_path).name}")
            
            try:
                # 读取文件内容
                file_content = self.file_utils.read_text_file(file_path)
                ...
                if file_path.endswith('.yaml') or file_path.endswith('.yml'):
                    ...
                    df = pd.DataFrame(filtered_rows, columns=['pattern', 'address', 'other', 'other2', 'other3'])
                
                # 收集逻辑规则
                all_logic_rules.extend(logic_rules)
                
                # 过滤df并合并到merged_by_type
                ...
                self.logger.warning(f"⚠️ 文件转换失败: {Path(file_path).name} - {str(e)}")
                converted_data.add_error(f"转换失败: {Path(file_path).name} - {str(e)}")
                continue
    ...
    def convert_downloaded_data(self, convert_download_results: Dict[str, 'DownloadedData']) -> Dict[str, ConvertedData]:
        """
        使用已下载的数据进行转换
        
        Args:
            convert_download_results: convert规则集的下载结果
            
        Returns:
            convert规则集名称到转换数据的映射
        """
        if not convert_download_results:
            self.logger.info("📋 没有convert下载数据，跳过转换阶段")
            return {}
        ...
        for i, (convert_name, download_data) in enumerate(convert_download_results.items(), 1):
            ...
            try:
                converted_data = self.convert_downloaded_ruleset(convert_name, download_data)
        ...
        self.logger.success(f"✅ 转换完成: {stats['successful_converts']}/{stats['total_converts']} 个规则集成功")
        
        return results
    
    def convert_all_rulesets(self) -> Dict[str, ConvertedData]:
        """
        转换所有convert配置中的规则集（原有方法，保持兼容性）
