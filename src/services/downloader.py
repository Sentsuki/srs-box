...
class DownloadService:
    ...
    def download_all_sources(self) -> Tuple[Dict[str, DownloadedData], Dict[str, DownloadedData]]:
        """
        统一下载所有数据源（rulesets + convert）
        ...
        total_sources = len(rulesets) + len(convert_config)
        
        self.logger.header("开始统一下载阶段")
        self.logger.info(f"📋 发现 {len(rulesets)} 个规则集 + {len(convert_config)} 个转换源，共 {total_sources} 个数据源")
        ...
        for ruleset_name, urls in rulesets.items():
            current_index += 1
            self.logger.step(f"下载规则集: {ruleset_name}", current_index, total_sources)
        ...
        for convert_name, urls in convert_config.items():
            current_index += 1
            self.logger.step(f"下载转换源: {convert_name}", current_index, total_sources)
            ...
            try:
                ...
                self.logger.error(f"❌ 转换源 {convert_name} 下载异常: {str(e)}")
        ...
        self.logger.success(f"✅ 下载完成: {total_successful}/{total_sources} 个数据源成功")
        self.logger.info(f"   规则集: {successful_rulesets}/{len(rulesets)} 成功")
        self.logger.info(f"   转换源: {successful_converts}/{len(convert_config)} 成功")
